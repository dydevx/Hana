...
The desktop hero uses a clean charcoal-and-photography split with a soft seam, a two-line headline and two primary visitor paths. Mobile puts the headline and both actions before a full-width photograph. The real logo is rendered from the supplied PDF into `assets/images/hana-flyer-logo.png` and used in the header and footer. Its distinctive colors remain intact. Menu categories use compact gold controls and card-based dish browsing. Booking uses an opaque charcoal form over dark restaurant photography. The footer closes the page with an image-led reservation callout, followed by a four-part directory for location, navigation, hours and contact. Legal and motion controls stay in a quiet final row.

The interior gallery uses a gap-free 12-column mosaic on desktop: one dominant room view, three supporting crops and a final panoramic trio. Tablet and mobile collapse to a balanced two-column flow with the lead image spanning both columns. Captions sit in a restrained image gradient and the existing lightbox remains keyboard accessible.

`assets/js/motion.js` owns progressive Web Animations API enhancements: headline entrance, photo pointer depth, interior reveal, gallery stagger, order-step emphasis, cart feedback and lightbox image changes. Native dialog entrance and button feedback use CSS. Checked reservation and pickup requests open as prefilled WhatsApp messages to the restaurant; nothing is sent until the visitor confirms in WhatsApp. No scroll hijacking or animation dependencies.
