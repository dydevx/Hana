...
test('Pickup excludes past dates, lead time, midday break and closing time',()=>{const now=new Date('2026-09-07T10:00:00Z');const slots=pickupSlots('2026-09-07',now);assert.equal(slots[0],'12:30');assert.ok(!slots.includes('14:30'));assert.ok(!slots.includes('16:00'));assert.ok(slots.includes('17:00'));assert.ok(!slots.includes('22:00'));assert.deepEqual(pickupSlots('2026-09-06',now),[]);assert.deepEqual(pickupSlots('bad',now),[]);});
// Synthetic test fixtures, NEVER published as restaurant menu data.
const items=new Map([['test-a',{id:'test-a',number:'TEST-1',name:'Testgericht',priceCents:123,variant:'Testvariante'}],['test-b',{id:'test-b',number:'TEST-2',name:'Testgericht B',priceCents:456}]]);
