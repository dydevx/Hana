...
const dishImages = new Set([
 'hana-3','hana-10','hana-13','hana-14','hana-15','hana-20',
 'hana-30a','hana-60a','hana-70a','hana-100a','hana-110a','hana-203','hana-205','hana-25',
 'hana-16','hana-17','hana-213','hana-214','hana-216',
 'hana-230','hana-231','hana-232','hana-233',
 'hana-242','hana-243','hana-245','hana-246','hana-248','hana-249',
 'hana-260','hana-263','hana-265','hana-266','hana-267','hana-270',
 'hana-280','hana-283','hana-284','hana-285','hana-286','hana-287','hana-291',
 'hana-310','hana-324',
 'hana-330','hana-332','hana-333','hana-335','hana-336','hana-338','hana-339','hana-340'
]);
const sceneDishImages = new Set([
 'hana-3','hana-10','hana-13','hana-14','hana-15','hana-20',
 'hana-30a','hana-60a','hana-70a','hana-100a','hana-110a','hana-203','hana-205','hana-25',
 'hana-310','hana-324'
]);
...
export function renderMenu(menu) {
 return menu.categories.map(cat => {
  ...
  return `<section class="menu-category${cat.note?' menu-category--with-note':''}" id="category-${e(cat.id)}"><h3>${e(cat.name)}${cat.quantityLabel?` <small>${e(cat.quantityLabel)}</small>`:''}</h3>${cat.note?`<p class="menu-category-note">${e(cat.note)}</p>`:''}<div class="dish-grid">${[...groups.values()].map(options => {
   const i = options[0];
   const photo=dishImages.has(i.id)?`<figure class="dish-media${sceneDishImages.has(i.id)?' dish-media--photo':''}"><img src="assets/images/menu-items/${e(i.id)}.webp" alt="${e(i.name)} aus der HANA Speisekarte" loading="lazy" decoding="async"></figure>`:'';
...
export function itemMeta(i) {
 return [i.allergens?.length?'Allergene (Speisekarte): '+i.allergens.join(', '):'',i.additives?.length?'Zusatzstoffe: '+i.additives.join(', '):''].filter(Boolean).map(e).join(' · ');
}
